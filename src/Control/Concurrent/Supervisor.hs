...
module Control.Concurrent.Supervisor
    (
      -- * Message queue
      --
      -- | 'Inbox' is specifically designed queue for implementing actor which is all behaviors available in this
    ...
    -- way to create a new 'Inbox'.  This limitation is intended to make it harder to access to read-end of 'Inbox' from
    -- other than inside of actor's message handler.
    --
    ...
    -- instances share single 'Inbox'.  This would be useful to distribute task stream to multiple worker
