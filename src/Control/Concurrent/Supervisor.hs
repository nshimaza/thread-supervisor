...
module Control.Concurrent.Supervisor
    ...
    {-|
        ...
        To protect read-end of the queue, separate types are given to read-end
        and write-end.  Message handler of actor can access to both end but only
        write-end is accessible from outside of message handler.  To realize
        this, constructor of message queue are not exposed.  The only way to
        create a new 'Inbox' object is creating a new actor using 'newActor'
        function.
    ...
    {-|
        ...
        * Bounded queue.
        ...
        The type 'Inbox' is intended to be used only for reading side as inbox
        of actor.  Single 'Inbox' object is only readable from single actor.  In
        order to avoid reading from other actors, no constructors are exposed
        but instead you can get it only via 'newActor' or 'newBoundedActor'.
    -}
    ...
    {-|
        ...
        read from given inbox directly.  You can write to given inbox with
        'sendToMe'.
    -}
    ...
    {-|
        ...
        > myActorHandler :: Inbox YourMessageType -> IO (
        > myActorHandler inbox = do
        >     newMessage <- receive inbox doSomethingWith newMessage
        >
